    ...
    <div className="h-full flex flex-col bg-white">
      <div className="border-b border-gray-100 p-3 flex items-center gap-2">
        ...
        <div className="min-w-0">
          <p className="text-sm font-medium text-gray-800 truncate">{conv.found_item.title}</p>
            ...
            <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[80%] rounded-2xl px-3.5 py-2 text-sm ${
                  mine ? "bg-trail-600 text-white rounded-br-sm" : "bg-gray-100 text-gray-800 rounded-bl-sm"
                }`}
              >
                {m.body}
              </div>
      ...
      <form onSubmit={handleSend} className="border-t border-gray-100 p-3 flex gap-2">
        <input
          ...
          className="flex-1 border border-gray-300 rounded-full px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-trail-500 focus:border-transparent"
        />
        <button
          type="submit"
          disabled={sending || !body.trim()}
          className="bg-trail-600 disabled:bg-gray-300 hover:bg-trail-700 text-white font-semibold px-4 rounded-full transition flex items-center justify-center"
        >
